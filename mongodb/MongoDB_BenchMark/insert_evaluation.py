...
with open('sample.csv', 'r') as rf:
    ...
    with open('insert.csv', 'w') as wf:
        ...
        for row in reader:
            data = {'id': str(id), 'field0': str(row[0]), 'field1': str(row[1]), 'field2': str(row[2]),
                    'field3': str(row[3]), 'field4': str(row[4]), 'field5': str(row[5]), 'field6': str(row[6]),
                    'field7': str(row[7]), 'field8': str(row[8]), 'field9': str(row[9]), 'field10': str(row[10]),
                    'field11': str(row[11]), 'field12': str(row[12]), 'field13': str(row[13]), 'field14': str(row[14])}
            id += 1
            # insert
            print('Insert: ' + str(id))
